...
//Last 16bit character is left out.
static uint8_t buffer[BUFFER_SIZE/*SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8*/] ={ //Cutting total buffer length by four. Attempt to send four packets.
 ...
 * @brief OLED_Update
 *
 * Update the OLED Buffer on the actual display, two pages at a time.
 */
...
//    I2C_Send(SSD1306_ADDR << 1);
//    I2C_Send(0x40);
//    for (i = line * SSD1306_LCDWIDTH; i < ((line+2) * SSD1306_LCDWIDTH); ) {
//
//        for (x = 0; x < 16; x++) {
//            I2C_Send(buffer[i]);
//            i++;
...
void OLED_Write_Text(int16_t x, int16_t y, char *text) {
    ...
    for (cnt = 0; cnt < length; cnt++){
        OLED_Write(x + ((cnt%16) * (cfont.x_size - 4)), y + ((cnt/16)*16), *text++); // x position was originally x + (cnt*(cfont.x_size))
    }
}
